...
namespace Stock.Infrastructure.Repositories
{
    public class ShowcaseRepository : Repository<Showcase>
    {
        ...
        {
            return base.GetAll().Include(s => s.ProductManagement).ThenInclude(p => p.Product).Include(s => s.ProductManagement).ThenInclude(p => p.Warehouse);
        }

        public override void Add(Showcase obj)
        {
            _context.Attach(obj.ProductManagement);
            _context.Attach(obj.ProductManagement.Product);
            _context.Set<Showcase>().Add(obj);
        }
